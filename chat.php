        ...
        <div class=chat-user-list>
          ...
          <ul class="chat-ui bsAccordion">
            <!--<li><a href=#>Favorites <span class="notification teal">4</span><i class=en-arrow-down5></i></a>
              ...
              <?PHP list($offline)=mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM users where online='1' and idDiv='$idv'")); ?>            
            <li><a href=1>Online <span class="notification green"><?PHP echo $offline; ?></span><i class=en-arrow-down5></i></a>
              <ul class=in>
                <?PHP 

                  $off= mysql_query("select * from users where online='1'");
                    ...
                    While($off2= mysql_fetch_assoc($off)) 

                      {
                        ...
                        $nik=$off2['idUser'];
                        $nik2= mysql_query("select * from emp where idUser='$nik'") or die(mysql_error());
                        $nik3= mysql_result($nik2, 0, 'fName');
                        $nik4= mysql_result($nik2, 0, 'lName');
                        $nik5= mysql_result($nik2, 0, 'avatar');                                               

                        echo '<li><a href=# class=chat-name>';

                          if($nik5=="") { echo '<img class=chat-avatar src=assets/img/avatars/11.png>';}
                          else { echo '<img class=chat-avatar src=assets/img/avatars/'.$nik5.' >'; }
              ...
              <?PHP list($offline2)=mysql_fetch_array(mysql_query("SELECT COUNT(*) FROM users where online='0' and idDiv='$idv'")); ?>
            <li><a href=2>Offline <span class="notification red"><?PHP echo $offline2; ?></span><i class=en-arrow-down5></i></a>
              <ul>
                <?PHP 

                  $off3= mysql_query("select * from users where online='0' and idDiv='$idv' ");
                    ...
                    While($off4= mysql_fetch_assoc($off3)) 

                      {
                        ...
                        $nik6=$off4['idUser'];
                        $nik7= mysql_query("select * from emp where idUser='$nik6'") or die(mysql_error());
                        $nik8= mysql_result($nik7, 0, 'fName');
                        $nik9= mysql_result($nik7, 0, 'lName');
                        $nik10= mysql_result($nik7, 0, 'avatar');                                               

                        echo '<li><a href=# class=chat-name>';

                          if($nik10=="") { echo '<img class=chat-avatar src=assets/img/avatars/11.png>';}
                          else { echo '<img class=chat-avatar src=assets/img/avatars/'.$nik10.' >'; }
...
<!--           <ul class="chat-ui chat-messages">
          ...
          <div class=chat-write>
            <form action=# class=form-horizontal role=form>
              <div class=form-group>
                <textarea name=sendmsg id=sendMsg class="form-control elastic" rows=0></textarea>
